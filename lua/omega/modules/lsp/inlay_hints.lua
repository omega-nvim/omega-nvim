...
function inlay_hints.new()
    inlay_hints.namespace = vim.api.nvim_create_namespace("textDocument/inlayHints")
    local self = setmetatable({ cache = {}, enabled = false }, { __index = inlay_hints })
...
local function clear_ns(bufnr)
    ...
    vim.api.nvim_buf_clear_namespace(bufnr, inlay_hints.namespace, 0, -1)
end

-- Disable hints and clear all cached buffers
function inlay_hints.disable(self)
    self.disable = false
    inlay_hints.disable_cache_autocmd()
...
local function set_all(self)
    for _, bufnr in ipairs(vim.api.nvim_list_bufs()) do
        inlay_hints.cache_render(self, bufnr)
    end
end

-- Enable auto hints and set hints for the current buffer
function inlay_hints.enable(self)
    self.enabled = true
    inlay_hints.enable_cache_autocmd()
    set_all(self)
end

-- Set inlay hints only for the current buffer
function inlay_hints.set(self)
    inlay_hints.cache_render(self, 0)
end

-- Clear hints only for the current buffer
function inlay_hints.unset()
    clear_ns(0)
end

function inlay_hints.enable_cache_autocmd()
    local group = vim.api.nvim_create_augroup("InlayHintsCache", {})
    vim.api.nvim_create_autocmd(
        { "BufWritePost", "BufReadPost", "BufEnter", "BufWinEnter", "TabEnter", "TextChanged", "TextChangedI" },
        {
            callback = function()
                require("omega.modules.lsp.on_attach").inlay_hints.cache()
            end,
            group = group,
        }
    )
    if opts.only_current_line then
        vim.api.nvim_create_autocmd({ "CursorMoved", "CursorMovedInsert" }, {
            callback = function()
                require("omega.modules.lsp.on_attach").inlay_hints.render()
            end,
            group = group,
        })
    end
...
function inlay_hints.disable_cache_autocmd()
    vim.cmd([[
...
local function parse_hints(result, bufnr)
    ...
    for _, value in pairs(result) do
        ...
        local line_len = string.len(vim.api.nvim_buf_get_lines(bufnr, line, line + 1, true)[1])
...
function inlay_hints.cache_render(self, bufnr)
    local buffer = bufnr or vim.api.nvim_get_current_buf()

    for _, v in pairs(vim.lsp.get_active_clients({ bufnr = buffer })) do
        v.request("textDocument/inlayHint", get_params(v, buffer), function(err, result, ctx)
            if err then
                return
            end
            ...
            inlay_hints.render(self, ctx.bufnr)
        end, buffer)
    end
end
...
local function render_line(line, line_hints, bufnr, max_line_len)
    ...
    if opts.max_len_align then
        ...
        virt_text = string.rep(" ", max_line_len - line_len + opts.max_len_align_padding)
    end
    ...
    if virt_text ~= "" then
        ...
        vim.api.nvim_buf_set_extmark(bufnr, inlay_hints.namespace, line, 0, {
...
function inlay_hints.render(self, bufnr)
    local buffer = bufnr or vim.api.nvim_get_current_buf()
